...
`audit:exposure-readiness` 會把曝光拆成四層檢查：搜尋發現、可投放推廣素材、詢價轉換路徑、自動化監控。GA4、GTM、Google Ads 與 LinkedIn Insight 這類外部平台 ID 會列為缺口，但預設不讓 CI 失敗；要把缺少外部追蹤也視為失敗時，使用 `audit:exposure-readiness:strict`。
