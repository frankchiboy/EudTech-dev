...
47. Static SEO HTML uses the generated social preview images for Open Graph and Twitter/X cards, with `og:image:alt`, `twitter:image:alt`, image width, and image height.
48. `image-sitemap.xml` lists the generated social preview images and no longer emits deprecated `image:title` or `image:caption` fields.
49. `npm run verify:social-images`, `npm run verify:seo-html`, `npm run verify:discovery`, and `npm run verify:live-exposure` validate social image size, format, page metadata, sitemap coverage, and production accessibility.
50. The configurator Share button prefers native Web Share API, falls back to clipboard copy, and adds share-specific UTM parameters while preserving the selected configuration.
51. First-party configurator lead events keep the shared configuration URL and share method so organic sharing can be measured before external ad-platform IDs are available.
52. `src/data/configuratorProductSeo.ts` covers all 11 currently public Comino device routes: `/configurator/5/`, `/configurator/13/`, `/configurator/21/`, `/configurator/22/`, `/configurator/23/`, `/configurator/27/`, `/configurator/28/`, `/configurator/29/`, `/configurator/30/`, `/configurator/34/`, and `/configurator/36/`.
53. Product-level exposure assets now cover 26 landing pages, 27 social preview images, 480 Google Ads keyword rows, 78 organic post rows, and 78 tracked UTM links.
