...
43. `npm run audit:external-platform-access` writes `reports/external-platform-access.json` with Netlify, Google ADC, Google Ads, LinkedIn, Meta, Microsoft Ads, GitHub, and 1Password access readiness for external tracking setup; GitHub and 1Password report `accessReady` separately from `configComplete`.
...
1. Keep `VITE_GTM_ID` and `VITE_GA_MEASUREMENT_ID` synced in Netlify and GitHub Actions to capture page views.
...
15. Use `npm run verify:marketing-platform-env:readonly`, `npm run verify:marketing-platform-env -- --op-item "EudTech Configurator Marketing Platforms"`, and `npm run audit:external-platform-access:readonly` for no-report read-only checks during investigation.
...
Google Search Console API submission is currently usable in this shell through local ADC when the scripts request the explicit `https://www.googleapis.com/auth/webmasters` scope. `npm run submit:search-console` has successfully submitted `sitemap-index.xml`, `sitemap.xml`, `image-sitemap.xml`, and `feed.xml`; `npm run monitor:sitemaps`, `npm run inspect:search-console`, and `npm run report:search-console` also complete without printing tokens. On 2026-07-06 Asia/Taipei, Search Console accepted all four sitemap/feed submissions with HTTP 204 and `monitor:sitemaps` reported `errors: 0`, `warnings: 0`. After the production deploy for commit `3c5fa0037ea638d88d946cde6e133aab9d091584`, the same four discovery files were resubmitted at 2026-07-06T02:16:39Z through 2026-07-06T02:16:41Z UTC and were downloaded by Google at 2026-07-06T02:16:41Z through 2026-07-06T02:16:43Z UTC with `errors: 0`, `warnings: 0`.

Production verification in this workspace has confirmed `/build-meta.json` can be matched against an expected deployed commit with `npm run verify:live-exposure -- --expect-commit <sha>`, and live exposure verification passed for commit `3c5fa0037ea638d88d946cde6e133aab9d091584`. The production site exposes sitemap, image sitemap, RSS feed, JSON Feed, LLM discovery files, static SEO HTML, social preview images, and the first-party marketing event endpoint. `npm run submit:indexnow:current` also successfully submitted the current URL set to IndexNow with HTTP 200 after that deploy.

Search Console URL Inspection on 2026-07-06T02:19:13Z inspected 65 URLs: 27 canonical URLs and 38 aliases. Google reported 16 indexed canonical URLs, 11 unindexed canonical URLs, 9 discovered-but-not-indexed canonical URLs, 1 stale double-slash user-canonical record from the 2026-06-27 crawl, and 0 inspection errors. The three URLs that Search Console reported without sitemap signal are present in both the live `sitemap.xml` and live `feed.xml`; the current action is to keep sitemap monitoring active and wait for Google to refresh those URL-level signals.

Search Console performance reporting is readable. For the 2026-04-05 through 2026-07-03 window, `npm run report:search-console -- --days=90 --row-limit=50` returned 21 configurator/solution rows, including impressions for `/configurator/`, `/configurator/28/`, `/solutions/gpu-server-quote/`, `/solutions/h200-vs-rtx-pro-6000/`, `/solutions/nvidia-h200-server/`, and `/solutions/rtx-pro-6000-workstation/`.

Netlify and GitHub Actions writes are currently available through the 1Password Automation item named `EudTech Configurator Marketing Platforms`. Netlify production env and GitHub Actions variables have `VITE_GTM_ID`, `VITE_GA_MEASUREMENT_ID`, `VITE_MARKETING_EVENT_ENDPOINT`, `VITE_META_QUOTE_EVENT_NAME`, and `VITE_MICROSOFT_UET_QUOTE_EVENT`. GitHub Actions secrets have `NETLIFY_AUTH_TOKEN`, `GOOGLE_ANALYTICS_PROPERTY_ID`, `GOOGLE_TAG_MANAGER_ACCOUNT_ID`, and `GOOGLE_TAG_MANAGER_CONTAINER_ID`. The production frontend bundle includes the configured GTM container and GA4 measurement ID.

External audit reports now distinguish `accessReady` from `configComplete`. Netlify, GitHub, 1Password, GA4, and GTM access are readable; `configComplete` remains false only because Google Ads, LinkedIn, Meta, and Microsoft Ads values are still empty.

The remaining external-access blockers are: Google Ads browser conversion ID and label, Google Ads developer token/customer/OAuth access, LinkedIn Insight partner ID and conversion ID, LinkedIn API access token/organization/ad account, Meta Pixel ID and API access, Microsoft Ads UET tag ID, Microsoft Ads developer token/customer/account/OAuth access, and the corresponding API-side Pixel/UET IDs. After these values are filled in 1Password, run the strict sync and audit commands in `docs/marketing-platform-onboarding.md`.
