...
import { ArrowRight, Blocks, Bot, CheckCircle2, ClipboardCheck, Cpu, FileSearch, FlaskConical, Radar, ShieldCheck, Workflow } from 'lucide-react';
...
const HomeSolutionsSection: React.FC<HomeSolutionsSectionProps> = ({ isEnglish }) => {
  const items = [
    ...
    { icon: Blocks, href: '/solutions/headless-saas', title: isEnglish ? 'Headless SaaS & Notion' : 'Headless SaaS 與 Notion', body: isEnglish ? 'Keep Notion and existing SaaS behind the work while adding a branded portal, events, AI, and governance.' : '保留 Notion 與既有 SaaS 工作方式，加入品牌入口、事件、AI 與治理。', accent: 'amber', color: 'bg-amber-400/15 text-amber-300 hover:border-amber-300/60' },
    ...
    <section className="bg-slate-950 py-20 text-white" aria-labelledby="home-solutions-heading">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="max-w-3xl">
          ...
          <h2 id="home-solutions-heading" className="mt-4 text-3xl font-bold tracking-tight sm:text-4xl">{isEnglish ? 'Four solution paths, one accountable delivery model' : '四條解決方案路徑，同一套可追蹤交付方式'}</h2>
