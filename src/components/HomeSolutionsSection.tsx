...
const HomeSolutionsSection: React.FC<HomeSolutionsSectionProps> = ({ isEnglish }) => {
  const items = [
    { visual: '/ai-agent/micro-illustrations/task-progression-v1.webp', href: '/solutions/ai-agent', title: isEnglish ? 'Agent SOR & Action Layer' : 'Agent SOR 與 Action Layer', body: isEnglish ? 'Keep ChatGPT or Claude as the interface. EudTech connects enterprise systems through MCP and records permissions, workflow state, approvals, actions, and audit evidence.' : '保留 ChatGPT／Claude 作為使用介面，由 EudTech 透過 MCP 串接企業系統，保存權限、流程狀態、核准、Action 與稽核證據。', color: 'hover:border-cyan-300/60' },
    ...
    <section className="bg-white py-20 text-slate-950 dark:bg-slate-950 dark:text-white" aria-labelledby="home-solutions-heading">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="max-w-3xl">
          ...
          <p data-home-hero-relationship className="mt-4 rounded-xl border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm leading-7 text-slate-700 dark:border-cyan-300/20 dark:bg-cyan-300/10 dark:text-slate-200">{isEnglish ? 'The homepage hero introduces EudTech as one delivery partner; these three paths cover governed Agent SOR implementation, AI infrastructure procurement, and social intelligence.' : '首頁主視覺介紹 EudTech 是同一個交付夥伴；以下三條路徑分別對應 Agent SOR 治理與執行層導入、AI 運算設備採購與社群情報需求。'}</p>
        </div>
