...
const ProductsOverviewPage: React.FC = () => {
  ...
  const groups = [
    {
      ...
      body: { zh: '從企業 AI Agent、金融資料到 AI 運算，依需求設計可串接、可追蹤、可驗收的導入方案。', en: 'From enterprise AI agents and financial data to AI computing, design connected, traceable, and testable implementations.' },
      icon: Workflow,
    ...
    },
    {
      ...
      body: { zh: '液冷工作站、4U 伺服器與多 GPU 平台，透過工作負載盤點與配置器建立實際報價需求。', en: 'Liquid-cooled workstations, 4U servers, and multi-GPU platforms with workload discovery and a quote-ready configurator.' },
      icon: Cpu,
    ...
    },
    {
      ...
      body: { zh: '帳號真實性、敘事、協調式行為與即時警示，支援品牌保護、公關、資安與公部門情報需求。', en: 'Profile authenticity, narratives, coordinated behaviour, and real-time alerts for brand protection, communications, security, and public-sector intelligence.' },
      icon: Shield,
  ...
  return <PageShell title={{ zh: '產品與品牌｜EudTech', en: 'Products and brands | EudTech' }} description={{ zh: '瀏覽 EudTech 軟體與 AI Agent 導入、Comino 液冷 AI 運算及 Cyabra 社群情報產品。', en: 'Explore EudTech software and AI Agent implementation, Comino liquid-cooled AI systems, and Cyabra social intelligence products.' }} path="/products">
    <PageHero eyebrow={{ zh: '產品與品牌', en: 'Products & brands' }} title={{ zh: '從產品能力直接進入導入或採購', en: 'Move from product capability to implementation or procurement' }} lead={{ zh: '每個產品分類都提供原廠能力、EudTech 交付方式、產品明細與明確的諮詢或配置入口。', en: 'Each product category connects vendor capabilities, EudTech delivery, product details, and a clear consultation or configuration path.' }} isEnglish={isEnglish} />
    <section className="py-20"><div className="mx-auto max-w-7xl space-y-12 px-6 lg:px-8">{groups.map(({ title, body, icon: Icon, href, image, imageAlt, imageContain, imageCaption, source, products }) => <article key={tx(title, isEnglish)} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="grid lg:grid-cols-[0.84fr_1.16fr]"><div className={`relative min-h-[280px] overflow-hidden ${imageContain ? 'bg-white p-6' : 'bg-slate-100 dark:bg-slate-950'}`}><img src={image} alt={tx(imageAlt, isEnglish)} loading="lazy" decoding="async" className={`h-full w-full ${imageContain ? 'object-contain' : 'object-cover'}`} /></div><div className="p-7 sm:p-9"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><Icon className="h-7 w-7 text-cyan-600 dark:text-cyan-300" /><h2 className="mt-5 text-2xl font-bold">{tx(title, isEnglish)}</h2><p className="mt-3 max-w-2xl leading-7 text-slate-600 dark:text-slate-300">{tx(body, isEnglish)}</p></div><Link to={href} className="inline-flex shrink-0 items-center text-sm font-semibold text-cyan-700 dark:text-cyan-300">{isEnglish ? 'View solution' : '查看解決方案'}<ArrowRight className="ml-2 h-4 w-4" /></Link></div><p className="mt-5 text-xs leading-5 text-slate-500 dark:text-slate-400">{tx(imageCaption, isEnglish)} <SourceLink href={source.href} label={source.label} isEnglish={isEnglish} /></p></div></div><div className="grid border-t border-slate-200 dark:border-slate-800 md:grid-cols-3">{products.map((product) => <Link key={product.id} to={`/products/${product.id}`} className="border-b border-slate-200 p-6 transition hover:bg-cyan-50 dark:border-slate-800 dark:hover:bg-cyan-950/30 md:border-b-0 md:border-r last:md:border-r-0"><h3 className="font-semibold">{product.title}</h3><p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{product.description}</p><span className="mt-4 inline-flex items-center text-xs font-semibold text-cyan-700 dark:text-cyan-300">{isEnglish ? 'Product details' : '產品詳情'}<ArrowRight className="ml-1 h-3 w-3" /></span></Link>)}</div></article>)}</div></section>
    <section className="border-y border-slate-200 bg-slate-50 py-16 dark:border-slate-800 dark:bg-slate-900/60"><div className="mx-auto grid max-w-7xl gap-8 px-6 md:grid-cols-3 lg:px-8">{[
      { title: { zh: '軟體先做流程診斷', en: 'Software starts with workflow discovery' }, body: { zh: '確認資料來源、權限、人工核准點與驗收指標後，再決定試點範圍。', en: 'Confirm data sources, permissions, human approvals, and acceptance metrics before defining the pilot.' } },
      { title: { zh: '硬體先做工作負載盤點', en: 'Hardware starts with workload discovery' }, body: { zh: '以模型、軟體、資料、使用者與場地條件形成可審查的配置。', en: 'Turn models, software, data, users, and site conditions into a reviewable configuration.' } },
      { title: { zh: '授權先確認使用方式', en: 'Licensing starts with the access model' }, body: { zh: '確認使用者、資料範圍、SaaS、Managed Service、地端或 API 的實際需求。', en: 'Confirm users, data scope, SaaS, managed service, on-premises, or API requirements.' } }
