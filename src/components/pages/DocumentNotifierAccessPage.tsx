...
const DocumentNotifierAccessPage: React.FC = () => {
  ...
  const releasePlan = useMemo(() => [
    ...
    ['02', '完整可檢視原始碼', '原始碼涵蓋通知、未讀清單、PDF 與資料夾開啟、本機 SQLite、去重與重試。測試資料與資料夾監看也一併提供。'],
    ['03', '限制存取連接器', 'WebJAgent、公文交換、憑證與正式收發功能為選用連接器。預設關閉，由使用機關自行設定。'],
    ['04', '安全發布', '每一版都執行機密與個人資料掃描、相依套件與授權檢查、測試、SBOM、SHA-256 與簽章。範例另由人員檢查。']
  ], []);
  ...
  const requestAccess = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!domainEligible(email)) { setNotice('申請信箱必須是結尾為 .gov.tw 或 .edu.tw 的機關信箱。'); setBusy(false); return; }
    try {
      ...
      setRequestId(payload.requestId); setStep('verify'); setNotice('驗證碼已寄到申請信箱，10 分鐘內有效。');
  ...
  const verifyAccess = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (payload.status === 'approved') { setDownloadUrl(payload.downloadUrl); setStep('approved'); setNotice('申請已核准。下載連結只能使用一次，15 分鐘後失效。'); }
      else { setStep('review'); setNotice('申請已完成驗證。EudTech 將依安全規則進行人員審查。'); }
