...
const ResourcesOverviewPage: React.FC = () => {
  ...
  const entryPoints = [
    {
      image: EDITORIAL_PHOTOGRAPHY.solutionDiscovery,
      eyebrow: { zh: "尚未確定規格", en: "Need help choosing" },
      title: {
        zh: "請 EudTech 協助選型",
        en: "Ask EudTech to recommend a direction",
      },
      body: {
    ...
    },
    {
      ...
      eyebrow: {
        zh: "已知道大致規格",
        en: "Know the approximate specification",
      },
    ...
    {
      image: EDITORIAL_PHOTOGRAPHY.governanceReview,
      eyebrow: {
        zh: "正在準備採購文件",
        en: "Preparing procurement documents",
      },
      title: {
        zh: "整理 RFQ 與驗收條件",
        en: "Prepare RFQ and acceptance requirements",
      },
      body: {
  ...
  const buyingChecks = [
    {
      ...
      body: {
        zh: "模型、軟體、資料量、使用人數與連續運作時間。",
        en: "Models, software, data scale, users, and sustained run time.",
      },
    },
    {
      n: "02",
      ...
      body: {
        zh: "桌邊或機架、供電、散熱、噪音、網路與搬運限制。",
        en: "Desk or rack, power, cooling, noise, network, and handling constraints.",
      },
    },
    {
      n: "03",
      ...
      body: {
        zh: "預算、供貨、交期、安裝、保固、付款與必要文件。",
        en: "Budget, availability, lead time, installation, warranty, payment, and required documents.",
      },
    },
    {
      n: "04",
      ...
      body: {
        zh: "硬體辨識、壓力測試、溫度、錯誤紀錄與指定工作負載。",
        en: "Hardware inventory, stress tests, thermals, error logs, and the agreed workload.",
      },
    },
  ];
  ...
  const groups = [
    {
      title: { zh: "選型與報價", en: "Selection and quoting" },
      kinds: ["solution"],
    },
    {
      title: { zh: "部署與散熱", en: "Deployment and cooling" },
      kinds: ["guide"],
    },
    {
      title: { zh: "比較與決策", en: "Comparison and decisions" },
      kinds: ["comparison"],
    },
    {
      title: { zh: "RFQ 與公部門採購", en: "RFQ and public procurement" },
      kinds: ["checklist"],
    },
  ];
  ...
  return (
    <PageShell
      title={{
        zh: "GPU 伺服器選型與採購｜EudTech",
        en: "GPU server selection and procurement | EudTech",
      }}
      description={{
    ...
    >
      <PageHero
        eyebrow={{
          zh: "GPU 伺服器選型與採購",
          en: "GPU selection and procurement",
        }}
        title={{
          zh: "不知道該買哪一台？先從需求開始",
          en: "Not sure which system to buy? Start with the requirement",
        }}
        lead={{
        ...
        imageAlt={EDITORIAL_PHOTOGRAPHY.governanceReview.alt}
        imagePosition={EDITORIAL_PHOTOGRAPHY.governanceReview.objectPosition}
        actions={
          <>
            <ActionLink href="/contact">
              {isEnglish ? "Ask EudTech to help choose" : "請 EudTech 協助選型"}
            </ActionLink>
            <ActionLink href="/configurator?request=true" secondary>
              {isEnglish ? "I know the specification" : "我已知道規格"}
            </ActionLink>
          </>
        }
      />
      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-3">
            {entryPoints.map(
              ({ image, eyebrow, title, body, href, action }) => (
                <Link
                  key={href}
                  to={href}
                ...
                >
                  <img
                    ...
                    decoding="async"
                  />
                  <p className="mt-6 text-xs font-semibold uppercase tracking-[0.14em] text-cyan-700 dark:text-cyan-300">
                    {tx(eyebrow, isEnglish)}
                  </p>
      ...
      <section className="border-y border-slate-200 bg-slate-50 py-20 dark:border-slate-800 dark:bg-slate-900/60">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-3xl font-bold tracking-tight">
              {isEnglish
                ? "Answer the four most common buying questions"
                : "先回答四個最常見的採購問題"}
            </h2>
          </div>
          ...
          <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
            <div>
              <h2 className="text-3xl font-bold tracking-tight">
                {isEnglish
                  ? "Four items to confirm before quoting"
                  : "報價前需要確認四項資訊"}
              </h2>
              <p className="mt-5 leading-8 text-slate-600 dark:text-slate-300">
                {isEnglish
              ...
              <p className="mt-7 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
                {isEnglish
                  ? "Source: official Comino product and technical documentation"
                  : "資料依據：Comino 原廠產品與技術文件"}
              </p>
              <div className="mt-3 flex flex-wrap gap-x-5 gap-y-3">
                <SourceLink
                  href={VENDOR_EVIDENCE.comino.sources.blackwell.href}
                  label={VENDOR_EVIDENCE.comino.sources.blackwell.label}
                  isEnglish={isEnglish}
                />
                <SourceLink
                ...
                />
                <SourceLink
                  href={VENDOR_EVIDENCE.comino.sources.downloads.href}
                  label={VENDOR_EVIDENCE.comino.sources.downloads.label}
                  isEnglish={isEnglish}
                />
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <details className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950 sm:p-8">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-xl font-semibold marker:hidden">
              <span>
                {isEnglish
                  ? "Browse all GPU procurement topics"
                  : "查看全部 GPU 採購專題"}
              </span>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-sm text-slate-600 dark:bg-slate-900 dark:text-slate-300">
                17
              </span>
            </summary>
            ...
            <div className="mt-8 grid gap-8 md:grid-cols-2">
              {groups.map(({ title, kinds }) => {
                const pages = CONFIGURATOR_SEO_PAGES.filter((page) =>
                  kinds.includes(page.kind || "solution"),
                );
                return (
                    ...
                    <ul className="mt-4 space-y-3">
                      {pages.map((page) => (
                        <li key={page.slug}>
                          <Link
                            to={`/solutions/${page.slug}`}
                            className="inline-flex text-sm leading-6 text-slate-600 underline decoration-slate-300 underline-offset-4 transition hover:text-cyan-700 dark:text-slate-300 dark:decoration-slate-700 dark:hover:text-cyan-300"
                          >
                            {tx(page.title, isEnglish)}
                          </Link>
                        </li>
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-7 px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div>
            <h2 className="text-3xl font-bold">
              {isEnglish
                ? "Bring the workload. EudTech will help define the system."
                : "提供工作負載，EudTech 協助確認設備方向"}
            </h2>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <ActionLink href="/contact">
              {isEnglish ? "Ask for selection help" : "請 EudTech 協助選型"}
            </ActionLink>
            <ActionLink href="/configurator?request=true" secondary>
              {isEnglish ? "Configure directly" : "直接建立配置"}
            </ActionLink>
          </div>
        </div>
      </section>
