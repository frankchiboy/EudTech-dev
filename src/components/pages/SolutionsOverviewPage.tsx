...
const SolutionsOverviewPage: React.FC = () => {
  ...
  const items = [
    ...
    {
      icon: Blocks,
      title: { zh: 'Headless SaaS 與 Notion 整合', en: 'Headless SaaS & Notion integration' },
      body: { zh: '保留 Notion 與既有 SaaS 作為資料及協作層，建立企業品牌的網站、客戶 Portal、事件流程與受控 AI Agent。', en: 'Keep Notion and existing SaaS as the data and collaboration layer while adding a branded website, customer portal, event workflows, and controlled AI agents.' },
      fit: { zh: '適合：團隊已用 Notion 或 SaaS 管理工作，但客戶需要更正式的品牌體驗、權限、自動化與服務邊界。', en: 'Best for teams already operating in Notion or SaaS whose customers need a stronger branded experience, permissions, automation, and service boundary.' },
      outputs: [{ zh: '品牌化網站或客戶 Portal', en: 'Branded website or customer portal' }, { zh: '已授權 API 與事件驅動整合', en: 'Authorised API and event-driven integration' }, { zh: '人員核准、版本與稽核證據', en: 'Human approval, version, and audit evidence' }],
      image: null,
      imageAlt: null,
      imageContain: false,
      visual: 'headless',
      source: VENDOR_EVIDENCE.notion.sources.overview,
      href: '/solutions/headless-saas'
    },
    {
      icon: Cpu,
  ...
  return <PageShell title={{ zh: 'AI 與數位服務解決方案｜EudTech', en: 'AI and digital service solutions | EudTech' }} description={{ zh: 'EudTech 提供 AI Agent 導入、Headless SaaS 與 Notion 整合、AI 運算基礎設施及社群情報解決方案。', en: 'EudTech provides AI Agent implementation, headless SaaS and Notion integration, AI infrastructure, and social intelligence solutions.' }} path="/solutions">
    <PageHero eyebrow={{ zh: '解決方案', en: 'Solutions' }} title={{ zh: '從實際需求選擇解決方案', en: 'Choose a solution from the outcome you need' }} lead={{ zh: 'EudTech 將流程自動化、Headless SaaS、GPU 運算設備與社群情報分成清楚的導入路徑，每條路徑都有內容、下一步與可追蹤交付。', en: 'EudTech separates workflow automation, headless SaaS, GPU infrastructure, and social intelligence into clear delivery paths with concrete content and next steps.' }} isEnglish={isEnglish} actions={<><ActionLink href="/contact">{isEnglish ? 'Discuss your need' : '說明您的需求'}</ActionLink><ActionLink href="/configurator?request=true" secondary>{isEnglish ? 'Start a configuration' : '開始配置詢價'}</ActionLink></>} />
    <section className="py-20"><div className="mx-auto max-w-7xl space-y-8 px-6 lg:px-8">{items.map(({ icon: Icon, title, body, fit, outputs, image, imageAlt, imageContain, visual, source, href }) => <article key={href} className="grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 lg:grid-cols-[0.92fr_1.08fr]"><div className={`relative min-h-72 ${imageContain ? 'bg-white p-6 dark:bg-slate-950' : 'bg-slate-100 dark:bg-slate-950'}`}>{visual === 'headless' ? <div className="absolute inset-0 flex items-center p-5"><HeadlessSaasVisual isEnglish={isEnglish} compact /></div> : image && imageAlt ? <img src={image} alt={tx(imageAlt, isEnglish)} loading="lazy" decoding="async" className={`absolute inset-0 h-full w-full ${imageContain ? 'object-contain p-6' : 'object-cover'}`} /> : null}<div className={`absolute inset-x-0 bottom-0 px-6 pb-5 pt-16 ${visual === 'headless' ? 'bg-gradient-to-t from-slate-100 via-slate-100/90 to-transparent dark:from-slate-950 dark:via-slate-950/90' : 'bg-gradient-to-t from-slate-950/85 to-transparent'}`}><SourceLink href={source.href} label={source.label} isEnglish={isEnglish} inverse={visual !== 'headless'} /></div></div><div className="p-7 sm:p-10"><Icon className="h-8 w-8 text-cyan-600 dark:text-cyan-300" /><h2 className="mt-5 text-3xl font-bold">{tx(title, isEnglish)}</h2><p className="mt-4 leading-8 text-slate-600 dark:text-slate-300">{tx(body, isEnglish)}</p><p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm leading-7 text-slate-700 dark:bg-slate-950 dark:text-slate-200">{tx(fit, isEnglish)}</p><ul className="mt-6 space-y-3">{outputs.map((output) => <li key={tx(output, isEnglish)} className="flex gap-3 text-sm leading-6 text-slate-600 dark:text-slate-300"><Check className="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" />{tx(output, isEnglish)}</li>)}</ul><Link to={href} className="mt-7 inline-flex items-center font-semibold text-cyan-700 dark:text-cyan-300">{isEnglish ? 'Open the complete solution' : '開啟完整解決方案'}<ArrowRight className="ml-2 h-4 w-4" /></Link></div></article>)}</div></section>
